...
gtk_telegraph_new_window (GApplication *app,
	...
	priv->chatsBox = GTK_LIST_BOX(gtk_builder_get_object(builder, "chatsBox"));
...
GtkWidget* makeContact(struct User* user) {
	...
	gtk_widget_set_halign (contactInfo, GTK_ALIGN_FILL);
	...
	GtkWidget* contactLastMessage = GTK_LABEL(gtk_label_new(""));
...
void putMessage(const gchar* text, gboolean local) {
